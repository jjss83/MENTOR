...
- The Mentor dashboard is auto-hosted by the API at `http://localhost:4173` (served via `http-server`). Use `dashboard-start`/`dashboard-status` to ensure it is reachable and point the user to that URL in a real browser (not the Simple Browser) for TensorBoard and health tiles.
